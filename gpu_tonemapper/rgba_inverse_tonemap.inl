...
const char* rgba_inverse_tonemap_shader = ""
    ...
    "layout(location = 5) uniform vec2 dimensions;                                                      \n"
    ...
    "   return adjPt;                                                                                   \n"
    "}                                                                                                  \n"
    "vec3 dither(vec2 uv, vec3 inColor)                                                                 \n"
    "{                                                                                                  \n"
    "   float noise[16] = float[16](  0.5000f, -0.0333f,  0.3667f, -0.1667f,                            \n"
    "                                -0.3000f,  0.2333f, -0.4333f,  0.1000f,                            \n"
    "                                 0.3000f, -0.2333f,  0.4333f, -0.1000f,                            \n"
    "                                -0.5000f,  0.0333f, -0.3667f,  0.1667f);                           \n"
    "                                                                                                   \n"
    "    vec3 outColor;                                                                                 \n"
    "    vec2 cr = mod(vec2(uv.x * dimensions.x, uv.y * dimensions.y), 4.0f);                           \n"
    "    int index = int(cr.y*4.0f + cr.x);                                                             \n"
    "    outColor = (floor(inColor*255.0f + 4.0f*noise[index]))/255.0f;                                 \n"
    "    return outColor;                                                                               \n"
    "}                                                                                                  \n"
    "                                                                                                   \n"
    "void main()                                                                                        \n"
    ...
    "float b = rgb.b;                                                                                   \n"
    "#endif                                                                                             \n"
    "vec3 tResult = texture(tonemapper, ScaleOffset(vec3(r, g, b), tSO)).rgb * rgb_premulalpha.a;       \n"
    "fs_color.rgb = dither(flipped, tResult);                                                           \n"
