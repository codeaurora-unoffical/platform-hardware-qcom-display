...
gralloc1_error_t BufferManager::LockBuffer(const private_handle_t *hnd, uint64_t usage) {
  std::lock_guard<std::mutex> lock(buffer_lock_);
  auto err = GRALLOC1_ERROR_NONE;
  ...
  if (err == GRALLOC1_ERROR_NONE && (hnd->flags & private_handle_t::PRIV_FLAGS_USES_ION) &&
  ...
  if (err == GRALLOC1_ERROR_NONE && CpuCanWrite(usage)) {
    private_handle_t *handle = const_cast<private_handle_t *>(hnd);
...
gralloc1_error_t BufferManager::UnlockBuffer(const private_handle_t *handle) {
  std::lock_guard<std::mutex> lock(buffer_lock_);
  auto status = GRALLOC1_ERROR_NONE;
  ...
  if (hnd->flags & private_handle_t::PRIV_FLAGS_NEEDS_FLUSH) {
    if (allocator_->CleanBuffer(reinterpret_cast<void *>(hnd->base), hnd->size, hnd->offset,
      ...
      status = GRALLOC1_ERROR_NO_RESOURCES;
    }
...
gralloc1_error_t BufferManager::AllocateBuffer(const BufferDescriptor &descriptor, buffer_handle_t *handle,
